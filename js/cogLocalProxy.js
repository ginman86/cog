	...
	function()
	{
		return {	block1:						
			...
			function(data, context, callback)
			{
				...
				csv	+= context.user.id + "," + context.user.gender + "," + context.user.age + "," + context.user.experience;
				csv += "\r\n\r\n";
			    ...
			    exportLink.appendChild(document.createTextNode(context.user.id + '_results.csv'));

			    $(exportLink).attr('download', context.user.id + '_results.csv');	//chrome ftw.

			    if (callback) {callback(exportLink);}			    
		    },
			programCode:
			function()
			{
